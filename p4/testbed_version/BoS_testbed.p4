...
control SwitchIngress(
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            hdr.aux_md.setValid();
            hdr.aux_md.egress_port = 0;
            hdr.aux_md.ground_truth = hdr.ethernet.src_addr[2:0];
            hdr.aux_md.pred = 0;
            hdr.aux_md.code = 0;
            hdr.aux_md.stored_escalation_flag = ESCALATION_NO;
            ig_tm_md.ucast_egress_port = pipe_aux_recir_port; // TODO: check
            ...
            if (!hdr.recir_md.isValid()) {
                ...
                // move to egress
                // hdr.aux_md.code = ig_md.code;
                // hdr.aux_md.stored_escalation_flag = ig_md.stored_escalation_flag;
            }
        } else {
...
control SwitchEgress(
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            // if (eg_md.use_rnn == 1) {
            if (eg_md.code[3:3] == 1) {
                hdr.aux_md.pred = eg_md.final_class[2:0];
            } else {
                hdr.aux_md.pred = eg_md.per_packet_class[2:0];
            }

            // move from ingress to here
            hdr.aux_md.code = eg_md.code;
            hdr.aux_md.stored_escalation_flag = eg_md.stored_escalation_flag;
        } else {
...
control AuxEgress(
        ...
        inout egress_intrinsic_metadata_for_deparser_t eg_intr_dprs_md,
        inout egress_intrinsic_metadata_for_output_port_t eg_intr_oport_md) {
    #include "aux_eg_global_cnt.p4"
    apply {
        if (hdr.normal_md.isValid()) {
            tab_aux_eg_global_normal_pkt_cnt.apply();
        } else {
            hdr.ethernet.ether_type = ETHERTYPE_RECIRCULATE;
            hdr.recir_md.setValid();
            hdr.recir_md._unused = 0;
            tab_aux_eg_global_to_recir_pkt_cnt.apply();
        }
    }
}
...
control AuxIngress(
    ...
    #include "aux_ig_global_cnt.p4"

    apply {
        ig_dprsr_md.drop_ctl = 0;
        ig_tm_md.ucast_egress_port = hdr.aux_md.egress_port;
        
        hdr.normal_md.setValid();
        hdr.normal_md.pkt_type = PKT_TYPE_NORMAL;
        hdr.normal_md._unused = 0;

        tab_aux_ig_global_pkt_cnt.apply();

        if (hdr.aux_md.stored_escalation_flag == ESCALATION_YES) {
            tab_global_IMIS_ground_truth_cnt.apply();
        } else {
            // if (collision == 0) {
            if (hdr.aux_md.code[1:1] == 0) {
                // if (use_rnn == 0) {
                if (hdr.aux_md.code[3:3] == 0) {
                    tab_global_pre_analysis_ground_truth_cnt.apply();
                } else {
                    // tab_global_rnn_cnt.apply();
                    tab_global_rnn_ground_truth_cnt.apply();
                    // tab_global_rnn_pred_cnt.apply();
                    // if (hdr.aux_md.ground_truth == hdr.aux_md.pred) {
                    //     tab_global_rnn_tp_cnt.apply();
                    // }
                    tab_global_rnn_conf_mat_cnt.apply();
                }
            } else {
                // if (use_per_packet == 0) {
                if (hdr.aux_md.code[2:2] == 0) {
                    tab_global_collision_IMIS_ground_truth_cnt.apply();
                } else {
                    tab_global_perpacket_ground_truth_cnt.apply();
                    tab_global_perpacket_conf_mat_cnt.apply();
                }
            }
        }
        /*
